          ...
          >
            ...
            >
              <video
                ...
                class="video-box"
              ></video>
              <div v-if="isPlay" class="video-out" @click="isPlay = false">
                退出播放
              </div>
            </div>
      ...
      <div class="pic_slide">
        <div class="thumbnail-box">
          <i v-if="allNum > 5" class="btn btn-prev" @click="prev">
            <h-icon types="h-xiangzuo1" />
          </i>
          <i v-if="allNum > 5" class="btn btn-next" @click="next"
            ><h-icon types="h-xiangyou2"
          /></i>
          <div class="list">
            <div ref="owl" class="deli-owl" :style="{ right: owlRight + 'px' }">
              <ul class="slide_list clearfix">
                <li
                  v-for="(item, index) in productInfo.productMultiImage"
                  :key="index"
                  :class="inde == index ? 'item-cur' : ''"
                  class="slide_item"
                  @mouseover="
                    mouseOv(
                      index,
...
export default {
  ...
  created() {
    if (
      this.productInfo.productMultiImage &&
      this.productInfo.productMultiImage.length > 0
    ) {
  ...
  methods: {
    ...
    move(ev) {
      if (!this.isPlay) {
        ...
        this.oBigImg.style.left =
          percentX * (this.bWidth - this.imgWidth) + "px";
